...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x_h1, x_h2, cov1, cov2,L_zupt]=ZUPTaidedINS(u1,u2,zupt1,zupt2,k1,k2)
...
[L_zupts, L_nzs] = SolveLMI(k1,k2);%k1=4 k2=2.6  7,7
...
L_nz = L_nzs{1};
m = 1;
% u1f = lowpass(u1,1,100);
[tiltE1,rollE1] = Tilt_Estimation([-u1(1,:)',-u1(2,:)',-u1(3,:)'],u1(5,:),u1(4,:),0.3);%0.3
[tiltE2,rollE2] = Tilt_Estimation([-u2(1,:)',-u1(2,:)',-u2(3,:)'],u2(5,:),u1(4,:),0.3);
...
for k=2:N
    ...
    [x_h1(:,k) quat1]=Navigation_equations1(x_h1(:,k-1),u_h1,quat1,tiltE1(k),rollE1(k)); % Subfunction located further down in the file.
    [x_h2(:,k) quat2]=Navigation_equations1(x_h2(:,k-1),u_h2,quat2,tiltE2(k),rollE2(k)); % Subfunction located further down in the file.
...
%     p = attitude(2);
%     y = attitude(3);
%     pitch(k) = atan2(sin(x_h1(8,k)),cos(x_h1(8,k))); %-pi to pi
%     SWbound = pi/6;
%     offset = pi/10;
%     if pitch(k) < -SWbound-offset
%         L_zupt = L_zupts{1};
%     elseif pitch(k) > -SWbound+offset && pitch(k) < SWbound-offset
%         L_zupt = L_zupts{2};
%     elseif pitch(k) > SWbound+offset
%         L_zupt = L_zupts{3};
%     end
    x_h1(8,k) = tiltE1(k);
    x_h1(7,k) = rollE1(k);
    x_2(1,k) = x_2(1,k-1) + 1.36*cos(x_h1(9,k))*Ts;
    x_2(2,k) = x_2(2,k-1) + 1.36*sin(x_h1(9,k))*Ts;
...
%         end
        ...
        %pitch = atan2(f_u,f_w); 
        y = [zeros(2,1);pitch;];
        v3 = [1 0 0;0 0 1;0 0 0];
        p3 = [0 0 0;0 1 0;0 0 0]; 
        z23 = zeros(2,3);
        C = [z3 v3 p3 z3 z3];
        
        % find new L from bias if bias is too different
%         if k-m > 10000
%             m = k;
%             L_zupts = SolveLMI(x_h1(10,k-1),x_h1(12,k-1));
%             L_zupt = L_zupts{1};
%         end
        % apply L, observe states
        ...
        [x_h1(:,k) quat1]=comp_internal_states(x_h1(:,k),dx_new,quat1);    
        x_h1(8,k) = tiltE1(k);
        x_h1(7,k) = rollE1(k);
        bias = x_h1(10:15,k);
        % Update the filter state covariance matrix P.
...
%         C(1,10) = 1;
%         C(2,12) = 1;
%         dx=L_nz*(y-C*x_h1(:,k)-9.8*[sin(x_h1(8,k));cos(x_h1(8,k))])*Ts;
%         
%         p_a=atan2(f_u,sqrt(f_v^2+f_w^2));
%         ge = sqrt(f_u^2 + f_w^2);
%         %p_a = sign(f_u)*acos(f_w/ge);
%         dp = 0.01 * (p_a - x_h1(8,k));
%         dx(1) = dp;
%         D = zeros(15,4);
%         D(8,1) = 1;
%         D(10,2) = 1;
%         D(12,3) = 1;
%         D(14,4) = 1;
%         [x_h1(:,k) quat1]=comp_internal_states(x_h1(:,k),D*dx,quat1); 
%         end
    end
    if zupt2(k)==true;
...
%         end
        ...
        % Correct the navigation state using the estimated perturbations. 
        % (Subfunction located further down in the file.)
        [x_h2(:,k) quat2]=comp_internal_states(x_h2(:,k),dx_new,quat2);    
        x_h2(8,k) = tiltE2(k);
        x_h2(7,k) = rollE2(k);
        bias = x_h2(10:15,k);
        % Update the filter state covariance matrix P.
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [y,q]=Navigation_equations1(x,u,q,pitch,roll)
...
Rb2t=q2dcm(q);
% roll
y(7)=atan2(Rb2t(3,2),Rb2t(3,3)); y(7) = roll;

% pitch
y(8)=-atan(Rb2t(3,1)/sqrt(1-Rb2t(3,1)^2)); y(8)=pitch;
...
function [y,q]=Navigation_equations2(x,u,q)
