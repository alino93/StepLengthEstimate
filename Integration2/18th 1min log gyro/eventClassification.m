...
azChest=aChest;
Yaw=yawC;
%events column 1: 0-unknown 1-stand 2-walking 3-sitting 4-turning 5-fall 6-lie down
...
while i < (length(t)-200)
    ...
    if Yawdif(i)>25                          %turning
        ...
        events(i:i+49,1) = ones(50,1)*3;
        i = i + 50;
    end
    if (axRV(i) > 1 || axLV(i) > 1)          %walking
        events(i:i+20,1) = ones(21,1)*1;
        i = i + 20;
    end
    if azCM(i) < -6                              %Bend detect: sit start or bend start and cont until stands again
